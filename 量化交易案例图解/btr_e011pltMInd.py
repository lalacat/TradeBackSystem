...
class TQSta001(bt.Strategy):
    ...
    def __init__(self):
        ...
        #        
        rsi = bt.indicators.RSI(self.datas[0],subplot=True)
        bt.indicators.SmoothedMovingAverage(rsi, period=10)
        
        # 可变权重均线，默认是子图模式：subplot
        bt.indicators.WeightedMovingAverage(self.datas[0], period=25,subplot=True)
        # ATR指标 默认是不绘制曲线：plot=False
        bt.indicators.ATR(self.datas[0], plot=True)
        #
        bt.indicators.StochasticSlow(self.datas[0])
        bt.talib.STOCH(self.data.high, self.data.low, self.data.close,
                                   fastk_period=14, slowk_period=3, slowd_period=3,plotname='TA_STOC')
        #
...
#TA-Lib — backtrader documentation https://www.backtrader.com/blog/posts/2016-07-26-talib-integration/talib-integration.html?highlight=plot