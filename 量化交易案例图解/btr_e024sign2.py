...
# 用于输出买卖执行信息
class sgn_st001(bt.SignalStrategy):
    
    def notify_order(self, order):
        # 检查订单order是否完成
        if order.status ==order.Completed:
            xss=''
            css=',成交价：{:.2f},小计 Cost: {:.2f},佣金 Comm %{:.2f}'.format(order.executed.price,order.executed.value,order.executed.comm) 
            if order.isbuy():
...
print('\n\t# 增加一个自定义信号策略')
print('\t# 用于输出买卖执行信息')
cerebro.signal_strategy(sgn_st001)
#
print('\n\t#2-4,添加broker经纪人佣金，默认为：千一')
cerebro.broker.setcommission(commission=0.001) #,commtype=bt.CommissionInfo.COMM_PERC,stocklike=True)
#
