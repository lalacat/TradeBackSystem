...
class TQSta001(bt.Strategy):
    #定义一个参数，对应next内置函数的周期间隔：exitbars
    params = (
        ('exitbars', 5),
    )
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                self.log('买单执行BUY EXECUTED, 成交价：%.2f,买入：%d 股,小计 Cost: %.2f,佣金 Comm %.2f'
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # 这个仓位设置模式，也可以修改相关参数，进行调整
            # 此处使用变量exitbars，代替经验参数，更加灵活，可以进行优化：
...
print('\t#2-1,起始资金 Starting Portfolio Value: %.2f' % dcash0)
#
#
...
t0str,t9str='2018-06-01','2018-12-31'
...
#
size = bt.sizers.FixedSize()
print('\n\t#2-5,设置每手交易数目为：10，不再使用默认值：1股')
cerebro.addsizer(bt.sizers.FixedSizeTarget)
#
